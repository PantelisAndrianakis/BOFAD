...
A `PostToolUse` hook runs `hooks/bofad-check.sh` after every file edit: a mechanical style check (tabs, Allman braces, comment spacing, no `var`, no functional chains, no switch arrows, no nullability annotations, one variable per line, no em dashes, blank-line and trailing-whitespace hygiene) on Java, C# and C/C++ files, the subset that survives the translation on Python and Rust files (Rust adds wrapped signatures and conditions, iterator pipelines and `iter()` in `for` loops, and untyped tuple bindings), plus dash and blank-line hygiene on markdown. Three Python companions deepen it where a grep cannot reach: `bofad-format.py` measures blank-line layout drift, `bofad-boxing.py` catches Java autoboxing waste and `bofad-lint.py` parses for switch shape (missing `default`, missing `break`, unbraced cases, too few cases, if/else chains that belong in a switch), missing braces, repeated getters, single-use locals, string concatenation in loops, naming (underscore fields, UPPER_CASE constants, no `I` interface prefix), inline fully qualified names, bare `{@link}` forms, multi-statement lines, lowercase comments, mixed line endings and the Oxford comma. On Python it parses instead for repeated calls, single-use locals, string concatenation in loops, missing type annotations, snake_case names, module-level constants that should be UPPER_CASE, semicolons and comment shape. On Rust it parses for `let` bindings without a written type, `unwrap` and `expect` outside tests, `use` group order, lowercase constants, multi-statement lines and comment shape. Missing python degrades every Python and Rust check to a silent skip. In hook mode findings are limited to uncommitted lines, so legacy files never trigger mass-reformat instructions. Violations are fed straight back to the model for immediate self-correction - instruction asks for compliance, the hook measures it and CI (`.github/workflows/bofad-check.yml`) keeps the checker honest against known-good and known-bad samples and holds SKILL.md under a 40KB size ceiling.
...
The plugin also ships three subagents: `bofad-wargame` refutes implementation plans before code is written; `bofad-voice-check` grades a reply against a probe rubric from `tests/voice/probes.md`; `bofad-code-check` grades finished code against the semantic rules the checker scripts cannot see (Solution ladder, allocation habits, smallest diff, behavior preservation). `tests/samples/SemanticBad.java` and `SemanticGood.java` validate the lint script's performance rules in CI, `Bad.py` and `Good.py` do the same for the Python rules. The `/bofad-review` command runs both review layers on demand, the checker script then the code-check agent, over given paths or the uncommitted changes; findings only, fixes wait to be asked for.
